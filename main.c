...
int main()
{
/* Defining integers at the start */
    float a,b,c;
...
/* Gaining input from the user and using it in calculations. */
    ...
    scanf ("%f",&a);//Need the & sign to help scanf get address as it takes pointers

    printf("Please enter the value before x :");//Explains to use values needed
    scanf ("%f",&b);
    ...
    {
        answer1 = solution1 (d, b, a);//Uses d,b and a values in solution1 function
...
float absolute(float s)//Function to find absolute value (used for square root function)
{
    ...
    {
        s = (s*(-1));//Can define new parameter but don't need to
    }

    return s;//returns s as the output
}
